...
const spellDC = casterToken.actor.system.attributes.spelldc;
...
if (args[0] === "on") {
	...
	const triggerEffect = async (damageType) => {
		...
		const spellLevel = args[args.length - 1].efData.flags.dae.itemData.system.level;
...
if (args[0] === "off") {
	let item = targetActor.actor.system.items.find(i => i.name === spellName);
	if (!item)
		return;
