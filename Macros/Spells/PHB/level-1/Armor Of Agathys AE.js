...
if (args[0] === "on") {
	...
	await target.actor.createEmbeddedDocuments('Item', [{
		...
		system: {
			...
			damage: {
				parts: [
					[(itemData.system.level * 5).toString(), "cold"]
				]
			},
			level: 0,
			school: "abj",
	...
	await target.actor.update({
		system: {
			attributes: {
				hp: {
					temp: (target.actor.system.attributes.temp || 0) + itemData.system.level * 5
				}
			}
		}
	});
	...
	const item = target.actor.items.find(i => i.name === spellName);
	if (item) {
		target.actor.deleteEmbeddedDocuments('Item', [item.id]);
	}

	const itemData = args[1].efData.flags.dae.itemData;
	let newTemp = (target.actor.system.attributes.temp || 0) - itemData.system.level * 5;
	if (newTemp < 0)
		newTemp = 0;
